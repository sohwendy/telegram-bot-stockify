...
RSpec.describe 'FormatHelper' do
  ...
  it '#format_price' do
    ...
    value = hash[:data]['ABC']
    result = "#{hash[:data].keys.first} <b>$#{value[:amount]}</b> #{Emoji::CHART_WITH_DOWNWARDS_TREND}"\
             " #{value[:change_amount]}, #{value[:change_percent]}, #{value[:name]}\n"
  ...
  context '#format_stat' do
    ...
    it '#print_spec' do
      ...
      result = "#{Emoji::CHART_WITH_UPWARDS_TREND}  <b>$#{value[:amount]}</b>  "\
               "#{value[:change_amount]}  #{value[:change_percent]}\n"
