  ...
  bot.listen do |message|
    begin
      if message.text && MEMBERS.include?(message.from.id.to_s)
        ...
        when /^\/list/
          result = command.list(arg[1])
          bot.api.send_message(chat_id: message.chat.id, text: result || "Sorry #{message.from.first_name}, I cant find list #{arg[1]}", parse_mode: 'Markdown')
